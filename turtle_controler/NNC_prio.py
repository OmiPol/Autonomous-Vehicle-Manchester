    ...
    def prio_callback(self, data): 
        ...
        for detected in data.yolov8_inference:
            ...
            if(detected.class_name == "ahead_only" or detected.class_name == "turn_right_ahead" or detected.class_name == "turn_left_ahead") and (detected.confidence > 0.9):
                ...
                elif(detected.class_name == "turn_right_ahead") and (priority_turn < 2) and ((area >= 3200) or (area <= 1700 and area >= 1300)):
                    priority_turn = 1
                    self.pub_turn_sign.publish(String(data=detected.class_name))
                elif(detected.class_name == "turn_left_ahead") and (priority_turn < 1) and ((area >= 3200) or (area <= 1700 and area >= 1300)):
                    priority_turn = 0
                    self.pub_turn_sign.publish(String(data=detected.class_name))
            ...
            elif(detected.class_name == "red" or detected.class_name == "yellow" or detected.class_name == "green") and (detected.confidence > 0.55):
                #print(area)
                if(detected.class_name == "red") and (area >= 400):
                    priority_sema = 2
                    self.pub_sema_sign.publish(String(data=detected.class_name))
                elif(detected.class_name == "yellow") and (priority_sema < 2) and (area >= 400):
                    priority_sema = 1
                    self.pub_sema_sign.publish(String(data=detected.class_name))
                elif(detected.class_name == "green") and (priority_sema < 2) and (area >= 400):
                    priority_sema = 0
                    self.pub_sema_sign.publish(String(data=detected.class_name))
                else:
                    pass
